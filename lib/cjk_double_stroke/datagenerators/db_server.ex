defmodule CjkDoubleStroke.Datagenerators.DBServer do
  @moduledoc """
  Mnesia database server. Run this on a dedicated DB node.
  Start with: iex --sname db@localhost -S mix run --no-halt -e "CjkDoubleStroke.Datagenerators.DBServer.start_link([])"
  """
  ...
  def get_aelement_conway(variant), do: GenServer.call(__MODULE__, {:get_aelement_conway, variant})
  ...
  @impl true
  def handle_call({:get_aelement_conway, variant}, _from, state) do
    {:reply, do_map_lookup(:aelement_map, variant), state}
  end
  ...
  def handle_call(:load_static_data, _from, %{loaded: false} = state) do
    ...
    IO.write("  aelements... ")
    aelements = Readstaticfiles.read_aelements()
    write(:aelements, aelements)
    IO.puts("done (#{length(aelements)} entries)")
