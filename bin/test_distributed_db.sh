...
elixir --name db@127.0.0.1 -S mix run --no-halt -e "CjkDoubleStroke.Datagenerators.DBServer.start_link([])" > /tmp/db_node.log 2>&1 &
DB_PID=$!
...
elixir --name app@127.0.0.1 -S mix run -e '
  Node.connect(:"db@127.0.0.1") || raise("Failed to connect")
  ...
  # Extra verification
  IO.puts("Mnesia nodes: #{inspect(:mnesia.system_info(:running_db_nodes))}")

  if result == expected do
    IO.puts("✅ SUCCESS: ids_init_search(\"是\") returned the expected result")
    IO.puts("   → Data was retrieved through the distributed setup")
    System.halt(0)
  else
    IO.puts("❌ FAILURE")
    System.halt(1)
